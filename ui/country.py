...
def run_country():
    ...
    2018년~2024년까지의 각국 입국 데이터를 분석하여, **향후 2년간의 예상 입국 인원**을 제공합니다.  
    ✅ **성공 가능성이 높은 상위 5개 국가**  
    🚀 **새롭게 떠오르는 주목할 5개 국가**  
    선택한 달을 기준으로 **3개월 전후의 입국자 수 추이**를 살펴보고,  

    각국 여행객들이 원하는 특별한 경험을 준비해보세요!  
    한국의 **다채로운 축제**와 **사계절 맞춤형 여행 패키지**를 기획하여,  
    더 많은 글로벌 여행객을 매료시킬 기회를 만들어보세요. ✨✈️
    """)
...
#####################################################
    ...
    # ✅ 선택한 국가의 정보 출력
    st.subheader(f"🔎 {selected_country} 여행 정보")
    if selected_country and selected_country in country_info_df.index:
        ...
        if not filtered_values.empty:  # 값이 존재하는 경우
            ...
            if st.button("➡ 축제 정보 보기"):
                ...
                print(st.session_state["selected_year"], st.session_state["selected_month"], st.session_state["selected_country"], st.session_state["info"], st.session_state["expected_visitors"]) # 디버깅용 출력
